...
let rec string_of_expr e =
  ...
  | BeginEnd(es) -> "BeginEnd(" ^ String.concat ";"
  ...
  | SetField(e1,id,e2) -> string_of_expr e1 ^ "." ^ id  ^ "<=" ^
                          string_of_expr e2
  | IsEqual(e1,e2) ->  string_of_expr e1 ^ "=" ^ string_of_expr e2
  | IsGT(e1,e2) ->  string_of_expr e1 ^ ">" ^ string_of_expr e2
  | IsLT(e1,e2) -> string_of_expr e1 ^ "<" ^ string_of_expr e2
