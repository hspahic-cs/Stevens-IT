...
public:
    ...
    void insert(const iterator &it, const std::pair<K, V> &key_value) {
        ...
        Node<K, V> *inserted_node = new Node<K,V>(key, key_value.second);

        if(y == nullptr){
            root_ = inserted_node;
            inserted_node->parent = nullptr;
        }
...
private:
    ...
    void insert_fixup(Node<K, V> *z) {
        ...
        while(z->parent != nullptr && z->parent->color == RED){
            if(z->parent->parent->left != nullptr && z->parent == z->parent->parent->left){
                ...
                if(z->parent->parent->right != nullptr && z->parent == z->parent->parent->right){
    ...
    // Might need to make "if y->left != null" to every case since all nodes have nullptrs
    ...
    // Do we have to account for when the tree is empty?
    ...
    int diameter(Node<K, V> *node) const {
        ...
        return std::max(std::max(diameter(node->left), diameter(node->right)), 
    ...
    size_t sum_null_levels(Node<K, V> *node, size_t level) const {
        ...
        if(node->left == nullptr && node->right != nullptr){
            return sum_null_levels(node->right, level + 1) + level+1;
        }

        else if(node->left != nullptr && node->right == nullptr){
            return sum_null_levels(node->left, level + 1) + level+1;
        }
